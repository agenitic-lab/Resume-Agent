...
start_idx = content.find('    const renderStep3 = () => (')
...
new_step3 = '''    // UI constants for the new AI Optimization Screen
    ...
    const handleOptimizeWithAI = () => {
        toast.error('Optimization feature coming soon!');
    };
    ...
    const renderStep3 = () => (
        <div className="space-y-6 max-w-5xl mx-auto text-left py-6">
                        ...
                        <div className="border border-gray-200 rounded-xl bg-gray-50/50">
                            <div className="flex justify-between items-center p-4 border-b border-gray-200 bg-white rounded-t-xl">
                                ...
                                <label className="flex items-center gap-2 text-xs font-bold text-gray-500 cursor-pointer">
                                    <input type="checkbox" className="w-4 h-4 rounded text-brand border-gray-300 focus:ring-brand" defaultChecked />
                                    Select All
                                </label>
                            </div>
                            ...
                            <div className="p-4 space-y-3">
                                {analysisResult.recommendations && analysisResult.recommendations.map((rec, i) => (
                                    <div key={i} className="bg-white border border-gray-200 rounded-lg p-4 flex gap-4 transition-all hover:border-gray-300 shadow-sm">
                                        <input type="checkbox" className="w-4 h-4 mt-1 rounded text-brand border-gray-300 focus:ring-brand" defaultChecked />
                            ...
                            <div className="p-4 border-t border-gray-200 bg-white rounded-b-xl flex justify-center">
                                <Button onClick={handleOptimizeWithAI} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-6 py-2 rounded-lg flex items-center gap-2 shadow-sm">
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                                    Optimize with AI ({analysisResult.recommendations ? analysisResult.recommendations.length : 0})
                                </Button>
                            </div>
                        </div>
                    </div>
            ...
            <div className="flex justify-end mt-8 gap-4">
                ...
                <button onClick={() => setCurrentStep(4)} className="px-8 py-4 rounded-xl font-bold bg-gray-800 text-white hover:bg-black transition-colors flex items-center gap-2">
