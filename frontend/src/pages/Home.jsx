...
class Neuron {
  ...
  update() {
    ...
    // Random firing state
    if (!this.firing && Math.random() < 0.001) {
      this.firing = true;
      this.firingTimer = 1;
    }

    if (this.firing) {
      this.firingTimer -= 0.01;
  ...
  draw(ctx) {
    ctx.beginPath();
    ctx.arc(this.x, this.y, this.radius + (this.firing ? this.firingTimer * 3 : 0), 0, Math.PI * 2);
    ctx.fillStyle = this.firing
      ? `rgba(141, 163, 74, ${0.4 + this.firingTimer * 0.4})`
      : 'rgba(141, 163, 74, 0.25)';
    ctx.fill();

    if (this.firing) {
      ctx.shadowBlur = 15;
      ctx.shadowColor = 'rgba(141, 163, 74, 0.5)';
    } else {
      ctx.shadowBlur = 0;
    }
  }
