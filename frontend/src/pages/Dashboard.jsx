...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-primary text-primary p-8">
      ...
      <div className="mb-8 bg-surface border border-gray-200 rounded-[2.5rem] p-10 shadow-xl shadow-black/5 relative overflow-hidden group">
        <div className="absolute top-0 left-0 w-full h-1 bg-brand/10" />
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-8">
            <div className="w-16 h-16 bg-brand rounded-2xl flex items-center justify-center shadow-lg shadow-sm">
          ...
          <Link
            to="/new-optimization"
            className="px-6 py-3 bg-brand text-white font-semibold text-sm tracking-wide rounded-2xl hover:hover:bg-red-600 transition-all active:scale-95 shadow-xl shadow-brand-primary/10 flex items-center gap-3 whitespace-nowrap"
          >
            ...
            <>
              ...
              <div className="flex items-center justify-between px-4">
                ...
                <div className="relative">
                  <div className="w-28 h-28 rounded-full border-2 border-gray-100 flex items-center justify-center bg-secondary shadow-inner">
                    <div className="text-center">
                      <div className="text-3xl font-bold text-primary">{Math.round(latestRun.ats_score_before || 0)}</div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-brand font-bold text-xl">+{Math.round(latestRun.improvement_delta || 0)}</span>
                  </div>
                  <div className="text-[7px] font-black uppercase tracking-[0.3em] text-gray-500 mt-1">Optimization Yield</div>
                </div>

                {/* After Score */}
                <div className="relative">
                  <div className="w-28 h-28 rounded-full border-2 border-brand-primary/20 flex items-center justify-center bg-brand/5 shadow-lg shadow-brand-primary/5">
                    <div className="text-center">
                      <div className="text-3xl font-bold text-brand">{Math.round(latestRun.ats_score_after || 0)}</div>
                      <div className="text-xs text-brand/60 mt-1">After</div>
                    </div>
                  </div>
                </div>
              </div>
...
function StatCard({ icon, title, value, highlight }) {
  return (
    <div className="bg-surface border border-gray-200 rounded-[2rem] p-8 hover:border-brand-primary/30 transition-all shadow-lg shadow-black/5 group">
      <div className="flex items-center gap-4 mb-4">
        <div className="p-3 bg-secondary rounded-xl group-hover:bg-brand/5 transition-colors">
          {icon}
        </div>
        <p className="text-gray-500 text-xs font-medium">{title}</p>
      </div>
      <p className={`text-4xl font-bold tracking-tight ${highlight ? 'text-brand' : 'text-primary'}`}>
        {value}
      </p>
    </div>
  );
...
function HistoryItem({ title, date, improvement }) {
  return (
    <div className="flex items-center justify-between p-5 bg-secondary rounded-2xl hover:bg-primary transition-all border border-gray-100 group">
      <div className="flex items-center gap-4">
        <div className="w-2 h-2 bg-brand rounded-full animate-pulse shadow-[0_0_8px_rgba(141,163,74,0.4)]" />
        <div>
          <p className="text-primary font-semibold text-sm">{title}</p>
          <p className="text-gray-500 text-xs mt-0.5">{date}</p>
        </div>
      </div>
      <span className="text-brand font-bold text-lg">{improvement}</span>
    </div>
  );
}
